...
public class MemberLoginTest {
    ...
    void loginTest() throws Exception {
        ...
        String body = mockMvc.perform(
                post("/api/v1/member/token")
                    ...
                    .content(params)
        ).andDo(print())    // body 데이터의 토큰을 가져와 변환시켜 테스트
                .andReturn()    // 반환값
                .getResponse()  // 응답 객체
                .getContentAsString(Charset.forName("UTF-8"));

        JSONData data = om.readValue(body, JSONData.class); // JSON 문자열을 JAVA 객체로 변환
    ...
    @Test
    @DisplayName("미로그인 상태(토큰이 없는)에서 회원 전용 URL 접근 통제 테스트")
    void guestAccessTest() throws Exception {
        mockMvc.perform(get("/api/v1/member/info"))
                .andDo(print())
                .andExpect(status().isUnauthorized());  // 헤더 쪽의 status가 400인지 확인
    }
}
